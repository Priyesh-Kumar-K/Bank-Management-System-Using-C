...
#include <conio.h>								//For console input and output type of functions
...
//User defined functons are divided intp two parts : Minor Functions (which are used frequently to support execution of core porgram) and Major Functions (which are a part of core program).  Minor function definition from Line 12-25. Major function definitions from Line 27-40.
...
void admin()
{
    ...
    if(strcmp(uname,"ADMIN")==0&&strcmp(pass,"PRIYESH")==0) c=1;//if user name and passpord matches, the value of c is changed from 0 to 1. The purpose of changing value of c is seen @line 77 where the password is asked again if c!=0 i.e. if given password is invalid
    else
    {
        title();
        gotoxy(25,10); printf("Invalid User name or Password!\a");
...
void welcome()
{
    ...
    gotoxy(25,8); printf("A program by :");
    gotoxy(25,9);for(i=0;i<27;i++) printf("%c",196);
    gotoxy(33,11); printf("Priyesh Kumar. K");
    gotoxy(24,20);
